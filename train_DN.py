...
# -------------------------------------------------------------------------------------
from models.w_unet.DN_unet import *
Net = DNet
...
def train_augment(image, mask, index):
    ...
    return input, dir_from_border, dir_to_center, weight, mask, index
...
def train_collate(batch):
    ...
    return [inputs, dirs_from_border, dirs_to_center, weights, masks, indices]
...
def evaluate( net, test_loader, criterion ):
    ...
    for i, (inputs, dirs_from_border, dirs_to_center, weights, masks, indices) in enumerate(test_loader, 0):
        ...
        with torch.no_grad():
            ...
            dirs_from_border_pred, dirs_to_center_pred = net.forward( inputs )
            loss1 = criterion(dirs_from_border_pred, dirs_from_border, weights )
            loss2 = criterion(dirs_to_center_pred, dirs_to_center, weights )
            loss = loss1 + loss2
            # print('val_batch loss:', loss)

        batch_size = len(indices)
        test_loss += batch_size*np.array((
                           loss.cpu().data.numpy(),
                           loss1.cpu().data.numpy(),
                           loss2.cpu().data.numpy(),
                           0,
                           0,
                           0,
                         ))
...
def run_train():
    ...
    initial_checkpoint = \
        RESULTS_DIR + '/WNet-DN-2a/checkpoint/00033500_model.pth'
    ...
    while  i < num_iters:  # loop over the dataset multiple times
        ...
        net.set_mode('train')
        optimizer.zero_grad()
        for inputs, dirs_from_border, dirs_to_center, weights, masks, indices in train_loader:
            ...
            if i % iter_valid == 0:
                ...
                log.write('%0.4f %5.1f k %6.1f %4.1f m | %0.3f   %0.2f %0.2f %0.2f | %0.3f   %0.2f %0.2f %0.2f | %0.3f   %0.2f %0.2f %0.2f | %s\n' % (\
                         rate[0], i/1000, epoch, num_products/1000000,
