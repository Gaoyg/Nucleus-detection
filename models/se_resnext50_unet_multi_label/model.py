...
class FcnHead(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.foreground = nn.Conv2d(512, 1,   kernel_size=3, padding=1)
        self.cut        = nn.Conv2d(512, 1,   kernel_size=3, padding=1)
        # self.border     = nn.Conv2d(512, 1,   kernel_size=3, padding=1)
    ...
    def forward(self, f):
        ...
        #logits  = self.logit(f)
        foregrounds = self.foreground(f)
        cuts = self.cut(f)

        return foregrounds, cuts
...
class UNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        self.foregrounds, self.cuts = data_parallel(self.fcn_head, features)




    def criterion(self, foregrounds_truth, cuts_truth):
        ...
        #
        # self.loss = FocalLoss2d()(self.logits, labels_truth)

        #balanced
        foregrounds_weight = make_weight(foregrounds_truth)
        cuts_weight = make_weight(cuts_truth)

        self.foreground_loss = WeightedBCELoss2d()(self.foregrounds, foregrounds_truth, foregrounds_weight) \
                                + SoftDiceLoss()(self.foregrounds, foregrounds_truth)
        self.cut_loss        = WeightedBCELoss2d()(self.cuts, cuts_truth, cuts_weight) \
                                + SoftDiceLoss()(self.cuts, cuts_truth)
        self.loss = self.foreground_loss + self.cut_loss
