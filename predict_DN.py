...
def run_predict():
    ...
    out_dir = RESULTS_DIR + '/WNet-DN-2a'
    initial_checkpoint = \
        RESULTS_DIR + 'WNet-DN-2a/checkpoint/00049999_model.pth'
    ...
    for tag_name, do_test_augment, undo_test_augment, params in augments:
        ...
        ## setup  --------------------------
        tag = 'WNet_DN_2a_val_%s'%tag_name   ##tag = 'test1_ids_gray2_53-00011000_model'
        ...
        for i in range(0,len(ids)):
            ...
            with torch.no_grad():
                input = torch.from_numpy(augment_image.transpose((2,0,1))).float().div(255).unsqueeze(0)
                input = Variable(input).cuda()
                dir_from_border_pred, dir_to_center_pred = net.forward(input)
            ...
            if 1:
                ...
                res_ = np.zeros((height, width, 3), np.float32)
                dir_from_border_pred = dir_from_border_pred[0].data.cpu().numpy()
                dir_from_border_pred = dir_from_border_pred[:, :height, :width]
                # print(dir_pred)
                dir_y = dir_from_border_pred[0] + 1
                dir_x = dir_from_border_pred[1] + 1
                res[:,:,0] = dir_y * 128 
                res[:,:,1] = dir_x * 128 

                dir_to_center_pred = dir_to_center_pred[0].data.cpu().numpy()
                dir_to_center_pred = dir_to_center_pred[:, :height, :width]
                # print(dir_pred)
                dir_y = dir_to_center_pred[0] + 1
                dir_x = dir_to_center_pred[1] + 1
                res_[:,:,0] = dir_y * 128 
                res_[:,:,1] = dir_x * 128 

            #save
            cv2.imwrite(out_dir +'/predict/%s/predicts/%s_border.jpg'%(tag, name), res)
            cv2.imwrite(out_dir +'/predict/%s/predicts/%s_center.jpg'%(tag, name), res_)
